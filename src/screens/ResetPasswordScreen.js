  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        // Backend: "A new password has been sent to your email."
        showMessage({ message: response.data.message, type: 'success' });
        // After success → go back to login screen
        navigation.goBack();
      } else {
        showMessage({ message: response.data.message || 'Something went wrong', type: 'danger' });
      }
    } catch (error) {
      showMessage({ message: error.response?.data?.message || 'Server error', type: 'danger' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <ScrollView contentContainerStyle={styles.container}>
          ...
          <Text style={styles.title}>{settings?.system_name || 'Forgot Password'}</Text>
          <Text style={styles.subtitle}>Enter your email and we’ll send you a new password</Text>
